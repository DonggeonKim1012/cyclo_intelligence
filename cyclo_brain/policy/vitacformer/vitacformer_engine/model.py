            ...
            _build_tactile_persistence(
                ...
                right_persistence=(
                    self.config.architecture_version not in (
                        _H200_ARCHITECTURE_VERSION, _POUR_H100_ARCHITECTURE_VERSION,
                    )
                ),
            )
        )
...
def _validate_contract(config: dict[str, Any]) -> None:
    ...
    if is_h200 or architecture == _POUR_H100_ARCHITECTURE_VERSION:
        recipe = (
            _H200_TRAINING_RECIPE_VERSION if is_h200
            else _POUR_H100_TRAINING_RECIPE_VERSION
        )
        if config.get("training_recipe_version") != recipe:
...
def _runtime_train_config(config: dict[str, Any], root: Path) -> dict[str, Any]:
    """Adapt the pour export in memory; never rewrite its hash-bound assets."""
    if config.get("architecture_version") != _POUR_H100_ARCHITECTURE_VERSION:
        return config
    ...
    if mismatches:
        raise ValueError(f"Unsupported ViTacFormer pour inference contract: {mismatches}")
    model_config = config.get("model_config")
    if not isinstance(model_config, dict):
        raise ValueError("ViTacFormer pour has no model_config")
    contract = {
    ...
    if "data_contract" in config and config["data_contract"] != contract:
        raise ValueError("ViTacFormer pour has a conflicting data_contract")
    source_commit = model_config.get("source_repo_commit")
    if config.get("source_repo_commit", source_commit) != source_commit:
        raise ValueError("ViTacFormer pour has a conflicting source_repo_commit")
...
def _validate_checkpoint_metadata(
    ...
    if architecture not in (_H200_ARCHITECTURE_VERSION, _POUR_H100_ARCHITECTURE_VERSION):
        return
...
def load_vitacformer_policy(
    ...
    weights_path = _checkpoint_path(root, explicit_weights)
