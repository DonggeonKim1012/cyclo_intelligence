...
ACTION_REQUEST_MODE_SYNC_PREFETCH = "sync_prefetch"
ACTION_REQUEST_MODES = {
    ...
    ACTION_REQUEST_MODE_SYNC_PREFETCH,
}
...
def normalize_action_request_mode(value: object) -> str:
    ...
    if mode in {ACTION_REQUEST_MODE_SYNC_PREFETCH, "prefetch_sync"}:
        return ACTION_REQUEST_MODE_SYNC_PREFETCH
...
class ControlLoop:
    ...
    def __init__(
        ...
        self._initial_pose_sync_action: Optional[np.ndarray] = None
        self._initial_pose_sync_source_chunk_size = 0
        self._generation = 0
    ...
    def configure(
        ...
        with self._lock:
            ...
            self._initial_pose_sync_hold_pending = False
            self._initial_pose_sync_action = None
            self._initial_pose_sync_source_chunk_size = 0
            self._real_first_action_pending = True
            self._real_first_publish_pending = True
    ...
    def start(self, publish_to_robot: Optional[bool] = None) -> bool:
        with self._lock:
            ...
            generation = self._generation
            sync_action = self._initial_pose_sync_action
            source_chunk_size = self._initial_pose_sync_source_chunk_size
            self._initial_pose_sync_action = None
            self._initial_pose_sync_source_chunk_size = 0
            self._running = False
            self._initial_pose_sync_in_progress = False
        ...
        if sync_action is None:
            if self._real_action_safety_enabled:
                raise RuntimeError(
                    "initial pose sync requires a completed Real safety preflight"
                )
    ...
    def preflight_start(
        ...
        *,
        ...
        with self._lock:
            ...
            prepare_initial_sync = (
                self._initial_pose_sync_enabled
                and publish_to_robot
                and not self._initial_pose_sync_completed
            )
            if prepare_initial_sync:
                self._initial_pose_sync_action = prepared[0].copy()
                self._initial_pose_sync_source_chunk_size = len(chunk)
                self._diagnostic_trace.record(
                    'chunk',
                    generation=self._generation,
                    preflight=True,
                ...
                logger.info(
                    "Real action preflight passed for initial pose sync "
    ...
    def _should_request_actions(self, processor: ActionChunkProcessor) -> bool:
        ...
        if self._action_request_mode in {
            ACTION_REQUEST_MODE_SYNC,
            ACTION_REQUEST_MODE_SYNC_STEP,
        }:
        ...
        if self._action_request_mode == ACTION_REQUEST_MODE_SYNC_PREFETCH:
