...
class ControlLoopSafetyTests(unittest.TestCase):
    ...
    def test_constructor_keeps_a_positional_order_before_b_only_knobs(self) -> None:
        params = list(inspect.signature(ControlLoop).parameters)

        self.assertLess(
            params.index("action_request_mode"),
            params.index("ordered_async_refill_margin_s"),
        )
        self.assertLess(
            params.index("refill_margin_s"),
            params.index("latency_warmup_samples"),
        )

    def test_sync_prefetch_mode_prefetches_inside_bounded_refill_window(self) -> None:
        processor = FakeProcessor(buffer_size=10)
        robot = FakeRobot()
        loop = self._make_loop(processor, robot)
        loop._action_request_mode = ACTION_REQUEST_MODE_SYNC_PREFETCH
    ...
    def test_sync_mode_preserves_legacy_empty_buffer_wait(self) -> None:
        processor = FakeProcessor(buffer_size=1)
        robot = FakeRobot()
        loop = self._make_loop(processor, robot)
        loop._action_request_mode = ACTION_REQUEST_MODE_SYNC
