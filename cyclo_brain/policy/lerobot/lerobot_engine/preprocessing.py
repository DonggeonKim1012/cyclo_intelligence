...
class PreprocessingMixin:
    ...
    def _build_observation(self, task_instruction: str) -> Dict[str, Any]:
        ...
        if target_joint_names:
            ...
            # Legacy flat-state tactile is opt-in through checkpoint metadata.
            # Do not infer tactile usage from a state-size mismatch alone.
            uses_flat_tactile_state = (
                tactile_state_mode(self._policy)
                == TACTILE_STATE_MODE_STATE_MEAN
            )
            if uses_flat_tactile_state and flat_state.size < expected:
