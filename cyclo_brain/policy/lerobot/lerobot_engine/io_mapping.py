...
class IoMappingMixin:
    ...
    def _init_robot(self, robot_type: str) -> None:
        ...
        policy_tactile_keys = self._policy_tactile_keys()
        self._uses_flat_tactile_state = (
            tactile_state_mode(self._policy) == TACTILE_STATE_MODE_STATE_MEAN
        )
        enable_tactile = bool(policy_tactile_keys) or self._uses_flat_tactile_state
    ...
    def _wait_for_policy_inputs(self, timeout: float) -> bool:
        ...
        if getattr(self, "_uses_flat_tactile_state", False):
            required_sensors.extend(getattr(self, "_tactile_modalities", []) or [])
    ...
    def _requested_camera_names(
        ...
        if not self._is_head_camera_policy_key(next(iter(policy_image_keys))):
            return policy_camera_names, ""

        head_camera = self._read_head_camera_profile_once()
        if head_camera not in _HEAD_CAMERA_NAMES:
            if policy_camera_names & {"scene", "cam_head"}:
                return set(), ""
            return policy_camera_names, ""
    ...
    @staticmethod
    def _is_head_camera_policy_key(policy_key: str) -> bool:
        suffix = str(policy_key or "")
        if suffix.startswith(_IMAGE_KEY_PREFIX):
            suffix = suffix[len(_IMAGE_KEY_PREFIX):]
        suffix = suffix.rsplit(".", 1)[-1].strip()
        if suffix in {"scene", "cam_head"}:
            return True
        match = _CAMERA_SEMANTIC_RE.match(suffix)
        if match:
            first = match.group("a")
            ...
            part = first if first in {"head", "wrist"} else second
            return part == "head"
        return suffix in _HEAD_CAMERA_NAMES
