...
def test_sh5_default_conversion_keeps_tactile_out_of_state_topics(tmp_path):
    converter = RosbagToLerobotConverterBase(
        ConversionConfig(
            repo_id="test",
            output_dir=tmp_path,
    ...
    assert "/left_hand/finger_pressures" not in converter.config.state_topics
    assert "/right_hand/finger_pressures" not in converter.config.state_topics
    assert converter._tactile_topic_key_map == {}


def test_selected_tactile_topics_require_tactile_mode(tmp_path):
    try:
        RosbagToLerobotConverterBase(
            ConversionConfig(
                repo_id="test",
                output_dir=tmp_path,
                selected_tactile_topics=["/right_hand/finger_pressures"],
            )
        )
    except ValueError as exc:
        assert "selected_tactile_topics require tactile_mode" in str(exc)
    else:
        raise AssertionError("selected tactile topics should require tactile mode")
...
def test_sh5_legacy_joint_states_fill_bilateral_state_and_full_action(tmp_path):
    ...
    assert state_messages[0][1].shape == (67,)
    np.testing.assert_allclose(
        state_messages[0][1],
        np.concatenate([
            full_positions[upper_body_indices],
            np.array([90.0, 91.0, 92.0, 93.0, 94.0], dtype=np.float32),
            np.array([100.0, 101.0, 102.0, 103.0, 104.0], dtype=np.float32),
        ]),
    )
    assert converter._state_joint_names[:57] == upper_body_state_names
    assert converter._state_joint_names[57:] == (
        [
            f"tactile_left_hand_pressure_finger_{i}_mean"
            for i in range(1, 6)
        ]
