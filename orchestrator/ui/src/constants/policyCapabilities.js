...
export const POLICY_USES_TACTILE_INPUT = {
  'lerobot:tactile_act': true,
  'lerobot:trex': true,
  'vitacformer:vitacformer': true,
};
...
export function usesTactileInput(serviceType, policyType) {
  const service = String(serviceType ?? '').trim().toLowerCase();
  const policy = String(policyType ?? '').trim().toLowerCase();
  return Boolean(POLICY_USES_TACTILE_INPUT[`${service}:${policy}`]);
}
...
export function resolveActionRequestMode(serviceType, policyType, requestedMode) {
  ...
  if (requested === 'sync_prefetch' || requested === 'prefetch_sync') {
